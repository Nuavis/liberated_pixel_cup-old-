var events = {keyDown:function(){},keyUp:function(){},mouseDown:function(){},mouseUp:function(){},mouseMove:function(){},mouseScroll:function(){}};
var key = [],mouse = {x:0,y:0,scroll:0,down:false,mDown:false};
function addEvents(){
    for (var i = 0;i<255;i++){
        key.push(0);
    }
    ...
    document.addEventListener("mousewheel",eScroll, true);
}
function eKeyDown(e){
    key[e.keyCode] = 1;
    events.keyDown(e);
...
var initialWheelScroll;
function eScroll(e){
    if (initialWheelScroll){
        e.delta = e.wheelDeltaY / initialWheelScroll;
        mouse.scroll += e.delta;
        events.mouseScroll(e);
    }else{
        initialWheelScroll = Math.abs(e.wheelDeltaY);
    }
}
